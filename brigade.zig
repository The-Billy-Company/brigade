//! brigade — a shard-aware unit-test runner for Zig.
//!
...
//! many the machine has. The suite this was written for reached ~1000 tests and
//! ~18 minutes of wall clock at `user/real = 0.64` on a 16-core box — the tests
//! were not slow, they were serial.
//!
...
//! `Brigade.shard` hangs n independent `Run` steps off one compiled binary and
...
//! order, so cost clusters by module (every differential in a file lands
//! together). Striding the residues spreads each cluster across all shards,
//! which is the balance a static split can get without a work queue.
//! `Brigade.shard` then asks for ~2x as many shards as cores, so the build
//! runner's own in-flight limit finishes the job a static split can't: a shard
//! that drew a cheap slice ends and the next one starts, rather than a core
//! idling beside a grinding neighbor.
//!
...
//! everything but these — how a `test-quick` tier stands aside a package's
...
//! at the same time. Key those per test and per pid. `BRIGADE_SHARD=0/1` (or
//! `-Dtest-shards=1`) restores a single-process run for bisecting a failure or
//! stepping under a debugger.
//!
//! Wired as a `.simple`-mode `test_runner`, which means the build runner judges
//! a shard by its exit code rather than the `std.zig.Server` protocol — that
//! protocol hands out one test index at a time and awaits its result, so it
//! cannot express a parallel run.
//!
...
/// diagnostics must therefore keep using `std.debug.print`, and the red-summary
/// path prints the one command that replays a shard in full. What belongs here
/// is what a test says when it *passed*.
///
/// Reached from a test body as `@import("root").note` — the test runner is the
/// root module of the binary the tests were compiled into.
...
///
/// Public because it *is* the `BRIGADE_SHARD` contract: what a spec means, and
/// which tests a given spec claims.
pub const Shard = struct {
    ...
    pub fn parse(spec: []const u8) ?Shard {
    ...
    pub fn owns(shard: Shard, test_index: usize) bool {
...
pub fn main(init: std.process.Init.Minimal) void {
    ...
    // Counted separately from `selected` so an empty run can name which lever
    // emptied it. A filter that matched plenty and a skip that then stood all
    // of it aside are different mistakes, and blaming the filter for the
    // skip's work sends the reader to the wrong variable.
    var matched: usize = 0;
    for (builtin.test_functions) |test_fn| {
        if (filter) |f| if (!f.matches(test_fn.name)) continue;
        matched += 1;
        if (skip) |s| if (s.matches(test_fn.name)) continue;
        owned += @intFromBool(shard.owns(selected));
        selected += 1;
    }
    // Owning nothing is ordinary — 31 of 32 shards do when a filter picks one
    // test. Selecting nothing *while a filter is set* is a typo, and it would
    // otherwise report as a green suite that tested exactly zero things. Every
    // shard fails on it, so the build fails whichever one the runner reaches
    // first. An unfiltered run is left alone: a package may legitimately have
    // no tests yet, and there is no name to have gotten wrong.
    if (selected == 0 and filter != null) {
        if (matched == 0) std.process.fatal(
            "BRIGADE_FILTER='{s}' matched none of the {d} tests",
            .{ filter.?.spec, builtin.test_functions.len },
        );
        std.process.fatal(
            "BRIGADE_FILTER='{s}' matched {d} of the {d} tests, and BRIGADE_SKIP='{s}' stood every one of them aside",
            .{ filter.?.spec, matched, builtin.test_functions.len, skip.?.spec },
        );
    }
...
///
/// Public because it *is* the `BRIGADE_FILTER` / `BRIGADE_SKIP` contract.
pub const Patterns = struct {
    spec: []const u8,

    pub fn matches(p: Patterns, name: []const u8) bool {
...
pub fn selects(filter: ?Patterns, skip: ?Patterns, name: []const u8) bool {
